...
def extract_fonts_from_zip(zip_path, font_folder, prefer_ttf, include_web):
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if not selected_fonts and not (include_web and web_files):
            return 0  # No font files found, skip this archive

        for font_path in selected_fonts:
            safe_copy(font_path, font_folder)

        for txt_path in license_files:
            safe_copy(txt_path, font_folder)

        if include_web:
            for web_path in web_files:
                web_folder = os.path.join(font_folder, 'web')
                safe_copy(web_path, web_folder)

        return len(selected_fonts) + (len(web_files) if include_web else 0)
...
def main():
    ...
    for file_name in os.listdir(args.input):
        if file_name.lower().endswith('.zip'):
            ...
            extracted = extract_fonts_from_zip(
                zip_path,
                font_folder,
                args.prefer_ttf,
                not args.no_web
